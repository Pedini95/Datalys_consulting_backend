...
class FacesMatching(db.Model):
    ...
    def find_one(id, is_deleted):
        actions_logs = SeamfixActions.query.filter_by(id=id, is_deleted=is_deleted).first()
        return actions_logs

    def find_by_libelle(libelle, is_deleted):
        actions_logs = SeamfixActions.query.filter_by(libelle=libelle, is_deleted=is_deleted).first()
        return actions_logs
    ...
    def get_by_criteria(criteria, index, size):
        query = SeamfixActions.query

        # Définir une liste de conditions
        conditions = [SeamfixActions.is_deleted == False]
        if 'id' in criteria:
            conditions.append(SeamfixActions.id == criteria['id'])
        if 'search_string' in criteria:
            conditions.append(SeamfixActions.search_string.like(f"%{criteria['search_string']}%"))
        ...
        if 'date_param' in criteria:
            ...
            if start_date_converted or end_date_converted:
                if operator == "[]" and start_date_converted and end_date_converted:
                    ...
                    conditions.append(and_(SeamfixActions.created_at >= start_date_converted, SeamfixActions.created_at <= end_date_converted))
                elif operator == "][" and start_date_converted and end_date_converted:
                    conditions.append(SeamfixActions.created_at.not_between(start_date_converted, end_date_converted))
                elif operator == ">" and start_date_converted:
                    conditions.append(SeamfixActions.created_at > start_date_converted)
                elif operator == "<" and end_date_converted:
                    conditions.append(SeamfixActions.created_at < end_date_converted)
                elif operator == ">=" and start_date_converted:
                    conditions.append(SeamfixActions.created_at >= start_date_converted)
                elif operator == "<=" and end_date_converted:
                    conditions.append(SeamfixActions.created_at <= end_date_converted)
        ...
        query = query.order_by(SeamfixActions.id.desc())
